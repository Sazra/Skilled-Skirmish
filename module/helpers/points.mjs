 ...
 * flat, Active-Effect-driven bonus (system.actionPoints.bonus), plus any
 * Lehren bonus targeting Action Points (see helpers/lehren.mjs).
 * @param {Actor} actor
 * @return {number}
 */
export function computeMaxActionPoints(actor) {
  ...
  const lehrenBonus = computeLehrenTargetBonus(actor, 'actionPoints');
  return Math.max(0, Math.round(3 + Math.min(reflexesLevel, 3) + bonus + lehrenBonus));
}
 ...
 * bonus (system.reactionPoints.bonus), plus any Lehren bonus targeting
 * Reaction Points (see helpers/lehren.mjs).
 * @param {Actor} actor
 * @return {number}
 */
export function computeMaxReactionPoints(actor) {
  ...
  const lehrenBonus = computeLehrenTargetBonus(actor, 'reactionPoints');
  return Math.max(0, Math.round(1 + Math.floor(maxActionPoints / 2) + Math.max(0, reflexesLevel - 3) + bonus + lehrenBonus));
}
