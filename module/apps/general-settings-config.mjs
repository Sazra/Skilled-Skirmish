...
/**
 * GM-only settings menu app bundling the system's five simple top-level
 * world settings (Skill Points for Level 10/5 Skills, Max Carry Weight
 * Formula, Haltbarkeit/Durability enabled, Calendaria daily settlement
 * enabled) behind one menu button, instead of Foundry's core Settings
 * dialog rendering them as separate plain config:true fields at the bottom
 * of its own list - see helpers/settings.mjs, where all five are now
...
export class SKSKGeneralSettingsConfig extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext(options) {
    ...
    context.calendariaDailySettlementEnabled = game.settings.get('sksk', 'calendariaDailySettlementEnabled');
    return context;
  }
  ...
  static async #onSubmit(event, form, formData) {
    ...
    await game.settings.set('sksk', 'calendariaDailySettlementEnabled', !!data.calendariaDailySettlementEnabled);
  }
}
